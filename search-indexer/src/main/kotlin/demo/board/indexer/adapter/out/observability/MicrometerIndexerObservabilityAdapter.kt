...
// (여기서만 Micrometer를 알고, 서비스/설정은 포트 인터페이스만 봅니다 — board-service와 동일한 헥사고날 규칙).
//
// MeterRegistry는 Boot가 자동 구성한 빈으로, micrometer-registry-otlp가 물려 있어 메트릭이 OTLP로 Alloy→Mimir에
// push됩니다. 메트릭 이름은 board-service 관례대로 present-tense 동사로 두어 OpenMetrics 예약 접미사(_total/_bucket
