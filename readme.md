...
* **Lint / Coverage**: ktlint, **Kover** (커버리지 게이트 — 라인 최소 92%)
...
| Method | URI | Description | 인가 |
| :--- | :--- | :--- | :--- |
| `POST` | `/api/auth/signup` | 회원가입(username/password, ROLE_USER) | 공개 |
| `POST` | `/api/auth/login` | 로그인 → 액세스 토큰(JWT) 발급 | 공개 |
...
* **정합성 회복(재색인)**: 인라인 색인 누락이나 인덱스 재생성 시 `POST /api/boards/search/reindex`가 DB를 **키셋 페이지네이션으로 순회**하며 전체를 다시 색인합니다(한 번에 한 페이지만 메모리에 올려 대용량에서도 일정한 메모리). 색인은 **페이지 단위 벌크**(`BoardSearchPort.indexAll` → ES `saveAll`)로 수행해 건별 왕복을 없애고, 한 페이지가 실패해도 그 페이지만 건너뛰며 실패 건수를 집계합니다(결과 `ReindexResult(indexed, failed)`). **ROLE_ADMIN**(Bearer 토큰)으로 보호됩니다.
...
* **커버리지(Kover)**: [Kover](https://github.com/Kotlin/kotlinx-kover)로 커버리지를 측정하고, `koverVerify`가 **라인 최소 92%**를 게이트합니다(부트스트랩 `BoardApplication*`은 집계 제외). 현재 라인 커버리지는 약 94%입니다.
...
* **Tempo** : <http://localhost:3200> (트레이스 조회 — OTLP 수신은 내부망에서 Alloy로부터만)
...
헥사고날 규칙(의존성 방향·계층 순수성·엔티티 격리)을 **Konsist** 아키텍처 테스트로 강제하고, **Kover**로 커버리지를 라인 92% 게이트합니다. 규율이 아니라 CI에서 깨지는 회귀 방지 장치입니다. 인증/인가는 리액티브 Spring Security + 자체 발급 JWT로, 쓰기는 인증·운영 트리거(재색인·조회수 플러시)는 ROLE_ADMIN으로 보호하고, 접속 정보·비밀번호·JWT 키는 환경변수로 외부화했습니다. (자세한 내용은 위 **📊 Coverage & 아키텍처 규칙**)
