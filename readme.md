...
- 루트 `subprojects{}`가 Kotlin/JDK21/ktlint 공통 컨벤션을 적용합니다. `koverVerify`(커버리지 게이트)는 `board-service`에 적용됩니다.
...
│       ├── adapter
│       │   ├── in
│       │   │   ├── web               Controller(suspend), WebDto, GlobalExceptionHandler
│       │   │   └── batch             @Scheduled 트리거(아카이브·조회수 플러시·아웃박스 릴레이)
│       │   └── out
│       │       ├── persistence       R2DBC 엔티티/리포지토리/어댑터, Outbox 어댑터, SpringTransactionRunner
│       │       ├── messaging         KafkaEventPublisherAdapter (EventPublisherPort 구현)
│       │       ├── redis             조회수 버퍼 · 분산 락
│       │       ├── search            ES 검색 어댑터(reader) · BoardDocument · 재색인
│       │       ├── security          BCrypt · Nimbus JWT
│       │       └── observability     Micrometer 어댑터
│       ├── application
│       │   ├── port.in               UseCase 인터페이스 + Command
│       │   ├── port.out              Repository/Search/Outbox/EventPublisher/TransactionRunner 포트
│       │   └── service               BoardService · BoardSearchService · RelayOutboxService 등
│       ├── config                    Security · Kafka 프로듀서 · OTel · Clock · OpenApi
...
- I/O를 건드리는 단건 연산은 `suspend fun`, 다건 조회는 포트 경계에서 `Flow<T>`(절대 `List` 아님).
...
- **블로킹 세계의 드라이빙 어댑터**(`@Scheduled` 배치 트리거, `@KafkaListener` 컨슈머)는 그 경계 안에서만 코루틴으로 브리지합니다(`runBlocking` 또는 동기 호출) — 블로킹이 안쪽으로 새지 않습니다.
...
### 4. 엔티티 ↔ 도메인 엄격 분리
`@Table` R2DBC 엔티티 ↔ 도메인은 `BoardMapper`에서만, `@Document` ES 문서 ↔ 도메인은 `BoardDocumentMapper`(board-service)/`ElasticsearchBoardIndexAdapter`(search-indexer)에서만 변환합니다. 프레임워크 애노테이션이 도메인 모델로 새어 들어가지 않습니다.
...
관련 파일: `board_outbox`(db/schema.sql), `OutboxPersistenceAdapter`, `RelayOutboxService`, `OutboxRelayScheduler`, `KafkaEventPublisherAdapter`, `SpringTransactionRunner` / (consumer) `BoardChangedListener`, `BoardIndexService`, `ElasticsearchBoardIndexAdapter`.
...
| `POST` | `/api/auth/login` | 로그인, HS256 JWT 발급 → 200 | 공개 |
...
한글 형태소 전문검색은 **정본(R2DBC)과 분리된 ES 인덱스**로 제공되며, 색인 동기화는 [이벤트 파이프라인](#-이벤트-파이프라인-transactional-outbox--kafka--search-indexer)을 통합니다. 역할 분담이 명확합니다:
...
> 알려진 한계: HTTP 요청 → 아웃박스 기록 트레이스와 릴레이 폴링 → 발행 트레이스는 별도입니다(요청 컨텍스트를 아웃박스 행에 저장해 발행 시 복원하면 request→index까지 이을 수 있음 — 후속 과제). 컨슈머 랙 / DLQ / 아웃박스 백로그 메트릭도 후속.
...
3. **논블로킹 E2E** — WebFlux + 코루틴 + R2DBC, JDBC 전무. 블로킹 컨슈머/스케줄러는 어댑터 경계 안에서만.
...
10. **강제되는 아키텍처·품질 게이트** — Konsist + Kover 92% + ktlint, CI에서 검증.
