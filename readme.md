...
헥사고날 규칙(의존성 방향·계층 순수성·엔티티 격리)을 **Konsist** 아키텍처 테스트로 강제하고, **Kover**로 커버리지를 라인 85% 게이트합니다. 규율이 아니라 CI에서 깨지는 회귀 방지 장치입니다. 인증/인가는 리액티브 Spring Security + 자체 발급 JWT로, 쓰기는 인증·운영 트리거(재색인·조회수 플러시)는 ROLE_ADMIN으로 보호하고, 접속 정보·비밀번호·JWT 키는 환경변수로 외부화했습니다. (자세한 내용은 위 **📊 Coverage & 아키텍처 규칙**)
