...
* **트랜잭션 경계는 DB 접근에만 좁게** 둡니다. R2DBC가 자동 구성하는 **`ReactiveTransactionManager`** 는 `suspend` 함수에도 적용되지만, `BoardService`에는 **클래스 레벨 `@Transactional`을 두지 않습니다**. ES 색인·Redis 증가 같은 외부 부수효과가 DB 트랜잭션 안(그리고 커밋 전)에서 실행되면 (1) 트랜잭션이 열린 채 외부 왕복을 기다려 커넥션을 오래 점유하고 (2) DB 롤백 시 ES만 앞서 반영되는 정합성 위험이 생기기 때문입니다. 쓰기는 단일 문장(R2DBC 자동 커밋)으로 처리하고 색인은 **커밋 이후** best-effort로 수행하며, 순수 읽기 목록 조회에만 `@Transactional(readOnly = true)`를 둡니다.
...
* **커버리지(Kover)**: [Kover](https://github.com/Kotlin/kotlinx-kover)로 커버리지를 측정하고, `koverVerify`가 **라인 최소 85%**를 게이트합니다(부트스트랩 `ReactiveStreamApplication*`은 집계 제외). 현재 라인 커버리지는 약 94%입니다.
...
쓰기 경로는 DB 문장만 (단일 문장 자동 커밋) 처리하고, ES 색인·Redis 증가 같은 외부 부수효과는 **트랜잭션 밖(쓰기는 커밋 후)** 에서 best-effort로 수행합니다. 클래스 레벨 `@Transactional`을 두지 않아, 트랜잭션이 외부 왕복을 기다리며 커넥션을 점유하거나 DB 롤백 시 ES만 앞서 반영되는 정합성 위험을 없앴습니다. 순수 읽기 목록 조회에만 `@Transactional(readOnly = true)`를 둡니다. (자세한 내용은 위 **⚡ Reactive Conventions**)
