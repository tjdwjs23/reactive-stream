...
- **관측성**: Grafana **Alloy → Grafana Cloud(LGTM)** — metrics·logs·traces를 모두 OTLP로 push(로그에 trace_id, 메트릭에 exemplar), **두 서비스 모두** 계측 + Kafka 경계 분산 트레이싱
...
- **스트리밍 읽기**: `BoardBatchQueryPort.findStalePage()`가 키셋 페이지네이션 `List<Board>`를 페이지 단위로 흘려보내 전체를 메모리에 올리지 않습니다.
...
**두 서비스 모두** metrics·logs·traces를 OTLP로 단일 수집기 **Grafana Alloy**(:4318)에 push하고, Alloy가 **Grafana Cloud** OTLP 게이트웨이로 전달합니다(Basic auth, 스크레이프 없는 순수 OTLP push). 자체 호스팅 LGTM은 걷어냈습니다 — 저장·조회·상관은 Grafana Cloud(Mimir/Loki/Tempo)가 담당합니다.

- **service.name**으로 구분: `search-service` / `search-indexer`. Grafana Cloud 대시보드의 `$job` 드롭다운에 각각 나타납니다.
- **로그**: Spring Boot 4 네이티브 구조화 로깅(ECS JSON) + OTel logback appender → OTLP → Grafana Cloud. **trace/span id가 로그에 실려 로그↔트레이스 상관**.
- **트레이스(Kafka 경계 포함)**: Micrometer Tracing → OTLP → Grafana Cloud. **프로듀서(`KafkaTemplate.observationEnabled`) + 컨슈머(`containerProperties.observationEnabled`)** 관측을 켜 `traceparent`를 Kafka 헤더로 전파하므로, `search-service`의 릴레이 발행 span과 `search-indexer`의 소비 span이 **하나의 트레이스로 연결**됩니다.
...
- **메트릭 exemplar**: `management.tracing.exemplars.include`로 카운터/타이머/히스토그램 데이터 포인트에 활성 trace의 `trace_id`가 부착돼(Micrometer Tracing이 `ExemplarContextProvider` 자동 구성) Grafana에서 지연 스파이크 → 실제 트레이스로 바로 점프됩니다.
- 자격증명(instanceId/token/endpoint)은 Secret/env로 주입하며 레포에 커밋하지 않습니다(런북: `deploy/README.md`).
...
### Prerequisites
JDK 25, `colima` · `kind` · `helm`(+ `docker` CLI). `brew install colima kind helm`.

### 한 번에 실행
```bash
brew install colima kind helm       # 최초 1회
./deploy/up.sh                      # colima → kind → 이미지 빌드/주입 → helm → 롤아웃 대기, 한 방에 (멱등)
#   관측성 Alloy까지 함께:  ./deploy/up.sh --obs  (Grafana Cloud 자격증명 env 필요)
kubectl get pods -w                 # 6개(코어) 또는 7개(--obs) Ready 대기
```
...
> `--obs`를 주면 관측성 **Alloy 1파드**(→ Grafana Cloud 포워더)까지 함께 뜹니다(파드 7개). 켤 땐 Grafana Cloud 자격증명(`GRAFANA_CLOUD_OTLP_ENDPOINT`/`_INSTANCE_ID`/`_API_TOKEN`)을 env로 넘기세요 — 관측은 Grafana Cloud에서 조회합니다. 기본은 코어만(6개)이라 앱 OTLP export는 꺼집니다.
...
GitHub Actions(`.github/workflows/ci.yml`): **모든 브랜치 push + main PR**에서 JDK 25로 `ktlintCheck → test → koverVerify → koverHtmlReport`를 실행하고, 테스트/커버리지 리포트를 아티팩트로 업로드합니다(멀티모듈 경로).
...
6. **분산 관측성** — 두 서비스 OTLP → Alloy → Grafana Cloud, 로그 trace_id·메트릭 exemplar, Kafka 경계 트레이스 연결.
