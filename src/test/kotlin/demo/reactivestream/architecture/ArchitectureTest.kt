package demo.reactivestream.architecture
    ...
    StringSpec({
        ...
        "레이어 의존성은 안쪽으로만 향한다 (Adapter → Application → Domain)" {
            project.assertArchitecture {
                val domain = Layer("Domain", "demo.reactivestream.domain..")
                val application = Layer("Application", "demo.reactivestream.application..")
                val adapter = Layer("Adapter", "demo.reactivestream.adapter..")
        ...
        "도메인 계층은 Spring/영속성 프레임워크에 의존하지 않는다" {
            Konsist
                .scopeFromPackage("demo.reactivestream.domain..")
                .files
                .assertFalse { file ->
        ...
        "도메인 모델에는 영속성/ES 애노테이션이 새어 들어가지 않는다" {
            Konsist
                .scopeFromPackage("demo.reactivestream.domain.model..")
                .classes()
                .assertFalse { clazz ->
        ...
        "웹 컨트롤러는 서비스 구현이 아니라 UseCase(in-port)에 의존한다" {
            Konsist
                .scopeFromPackage("demo.reactivestream.adapter.in.web..")
                .files
                .assertFalse { file ->
                    file.hasImport { import -> import.name.startsWith("demo.reactivestream.application.service") }
                }
        }
        ...
        "R2DBC @Table 엔티티는 영속성 어댑터 패키지 안에만 존재한다" {
            Konsist
                ...
                .assertFalse { clazz -> !clazz.resideInPackage("demo.reactivestream.adapter.out.persistence..") }
        }
        ...
        "ES @Document 문서는 검색 어댑터 패키지 안에만 존재한다" {
            Konsist
                ...
                .assertFalse { clazz -> !clazz.resideInPackage("demo.reactivestream.adapter.out.search..") }
        }
    })
