...
> (WebFlux + 코루틴 + R2DBC)에서 측정된 기록입니다. 키셋 페이지네이션의 "깊이 무관성" 논지는 스택과
> 무관하게 유효하지만, 절대 수치는 당시 기록으로 보존합니다.
...
전제: 인프라 + 앱이 떠 있어야 한다(로컬 k8s는 `./deploy/up.sh`로 전체 기동, `search-service`는 `localhost:8080`). 빠른 개발 실행은 데이터스토어를 `kubectl port-forward`로 당긴 뒤 `./gradlew :search-service:bootRun`. 그리고 `board`에 대용량 데이터가 시드돼 있어야 한다.
