...
> **관측성 알려진 한계/후속 과제**: ① HTTP 요청→아웃박스 기록 트레이스와 릴레이 폴링→Kafka 발행 트레이스는 **분리**됩니다(비동기 경계가 outbox 테이블이라 traceparent가 자동 전파되지 않음 — 릴레이→컨슈머 구간은 연결됨). 완전한 요청→색인 계보가 필요하면 traceparent를 outbox 행에 저장해 발행 시 복원. ② `search-indexer` `@KafkaListener`에 **DLQ/에러 핸들러 미설정**(기본 핸들러가 재시도 후 스킵 — 포이즌 메시지는 조용히 유실). ③ **컨슈머 랙·아웃박스 미발행 백로그 메트릭/패널 부재**. ④ Grafana 대시보드는 HTTP 중심이라 Kafka/색인/아웃박스 패널이 없음.
