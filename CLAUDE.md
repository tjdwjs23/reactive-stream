...
- **원클릭**: `./deploy/up.sh`(코어 = 앱 2 + PostgreSQL/Redis/Elasticsearch/Kafka = 6파드, 멱등) / `./deploy/up.sh --obs`(+ 관측성 Alloy 1파드 = 7파드). 정리 `./deploy/down.sh`(`--all`이면 colima까지).
- **차트**: `deploy/helm/search-platform`(Chart/values/templates). 데이터스토어·앱은 항상, 관측성 **Alloy**(→ Grafana Cloud 포워더)는 `observability.enabled`로 토글(기본 off — 앱 OTLP export도 함께 꺼짐). Alloy 설정은 `files/alloy/config.alloy`. 자체 호스팅 LGTM은 걷어냈고, 3종 신호를 **Grafana Cloud** OTLP 게이트웨이로 보냅니다(자격증명은 Secret으로 주입, 아래 관측성 항목·`deploy/README.md`).
...
> **알려진 tradeoff(중복)**: 같은 `boards` 인덱스를 search-service(검색/재색인 reader)와 search-indexer(이벤트 writer)가 공유하므로, `BoardDocument`와 Nori 설정 JSON(`board-settings.json`/`board-mappings.json`)이 **두 모듈에 중복**됩니다(설정 JSON은 byte-identical, `BoardDocument.kt`는 package·주석만 다르고 스키마(애노테이션·필드)는 동일 — 컴파일 타임 결합이 없어 한쪽만 바꾸면 런타임 스키마 불일치로 드러남). 인덱스 생성 이니셜라이저도 양쪽에 각각 있습니다(둘 다 멱등). 매핑 변경 시 양쪽을 함께 고쳐야 하며, **공유 스키마 모듈 추출**(event-contract 선례처럼)이 후속 과제입니다. 또한 `reindex`는 search-service가 ES에 직접 쓰는 dual-writer 경로라(마지막 쓰기 승리 경합 가능), 이벤트 리플레이 기반으로 옮기는 것도 후속 과제입니다.

> **이벤트 파이프라인 관측성(구현됨)**: ① `search-indexer`는 배치 `@KafkaListener` + **DLQ/에러 핸들러**를 둡니다 — `DefaultErrorHandler` + `DeadLetterPublishingRecoverer`가 재시도 후 실패 레코드를 `board-changed-dlq`로 격리해 조용한 유실/무한 재시도 정체를 막습니다(`KafkaConsumerConfig`). DLQ로 격리되는 레코드는 발행 시 **`board.indexer.dlq` 카운터**로 집계돼(리졸버 콜백에서 증가) 격리가 조용히 묻히지 않습니다. ② **컨슈머 랙 메트릭**(`kafka.consumer.*` records-lag, `MicrometerConsumerListener` 바인딩)과 **아웃박스 미발행 백로그 게이지**(`board.outbox.unpublished`, 릴레이 사이클마다 `RelayOutboxService`가 갱신)를 노출합니다. ③ `search-indexer`는 search-service와 같은 **헥사고날 관측 포트**(`IndexerObservabilityPort` ← `MicrometerIndexerObservabilityAdapter`)로 색인 도메인 메트릭을 냅니다 — 반영 문서 수(`board.indexer.indexed`/`board.indexer.deleted`)와 배치 ES 벌크 쓰기 지연(`board.indexer.batch` 타이머, `publishPercentileHistogram`). Grafana "이벤트 파이프라인" 행에 컨슈머 랙·아웃박스 백로그·소비 처리량 + **DLQ 격리·색인 처리량(upsert/delete)·색인 배치 지연(avg/p95/p99)** 패널이 있습니다(대시보드는 `deploy/grafana-cloud/dashboards/hexagonal-app.json` — Grafana Cloud에 import). **외부 자원 장애는 서킷브레이커**로 빠르게 실패시킵니다(위 회복탄력성 항목 참조).
>
> **텔레메트리 파이프라인(Alloy → Grafana Cloud)**: 두 서비스는 metrics/logs/traces 3종을 OTLP로 로컬 **Alloy**(:4318)에 push하고, Alloy가 **Grafana Cloud** OTLP 게이트웨이(`.../otlp`)로 Basic auth 전달합니다(자체 호스팅 LGTM은 걷어냄). Alloy 설정 `deploy/helm/search-platform/files/alloy/config.alloy`, 자격증명(`GRAFANA_CLOUD_INSTANCE_ID`/`_API_TOKEN`/`_OTLP_ENDPOINT`)은 Secret/env로 주입(레포에 커밋 금지). **로그의 trace_id**는 ECS-JSON + MDC `trace.id`/`span.id`로 실려 로그↔트레이스 상관이 됩니다. **메트릭 exemplar**: `management.tracing.exemplars.include`로 카운터/타이머/히스토그램 데이터 포인트에 활성 trace의 trace_id가 부착돼 Grafana에서 메트릭→트레이스 점프가 됩니다(Micrometer Tracing이 `ExemplarContextProvider`를 자동 구성, 별도 코드 없음 — 히스토그램 버킷에 실리므로 `percentiles-histogram`을 함께 켬).
>
