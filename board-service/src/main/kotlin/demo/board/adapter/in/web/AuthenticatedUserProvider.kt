...
// 현재 인증된 사용자의 정보(id + 관리자 여부)를 (서블릿) 보안 컨텍스트에서 꺼냅니다.
// 리소스 서버가 검증한 principal은 Jwt이며, 발급 시 sub에 사용자 id를, roles 클레임으로 권한을 넣었으므로
// subject를 id로, 권한 목록에 ROLE_ADMIN이 있는지로 관리자 여부를 판별합니다.
// MVC + 가상 스레드 스택이라 SecurityContextHolder(ThreadLocal 기반)에서 동기로 읽습니다.
// 컨트롤러가 작성자 id·요청자 인가 정보를 얻는 seam으로, 표준 MockMvc 단위 테스트에서는
// 이 컴포넌트를 목으로 주입하거나 SecurityMockMvcRequestPostProcessors.jwt()로 컨텍스트를 채워 검증할 수 있습니다.
@Component
class AuthenticatedUserProvider {
    ...
    fun currentUserId(): Long = current().id
