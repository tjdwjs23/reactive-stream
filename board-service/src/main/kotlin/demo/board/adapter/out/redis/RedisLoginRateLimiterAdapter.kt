...
// LoginRateLimiterPort의 Redis 구현(리액티브 Lettuce, 논블로킹). username별 실패 횟수를 고정 윈도우로 셉니다.
//
//  - recordFailure: INCR login:fail:{key}. 값이 1이면(첫 실패) EXPIRE로 윈도우 TTL을 겁니다 → 윈도우가 흐르면
...
class RedisLoginRateLimiterAdapter(
    ...
    override suspend fun isBlocked(key: String): Boolean {
        val current =
            redis
                .opsForValue()
                .get(redisKey(key))
    ...
    override suspend fun recordFailure(key: String) {
        val k = redisKey(key)
        val count = redis.opsForValue().increment(k).awaitSingleOrNull() ?: 0L
        // 첫 실패에만 윈도우 TTL을 건다(이후 INCR은 TTL을 갱신하지 않아 고정 윈도우가 유지됨).
        if (count == 1L) {
            redis.expire(k, Duration.ofMinutes(windowMinutes)).awaitSingleOrNull()
        }
    }

    override suspend fun reset(key: String) {
        redis.delete(redisKey(key)).awaitSingleOrNull()
    }
