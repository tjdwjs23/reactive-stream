// search-service = 게시판 애플리케이션(정본 DB) + Outbox 프로듀서.
...
// (docker-compose의 빌드 컨텍스트와 동일 표기). Gradle 기본 테스트 작업 디렉터리는 서브모듈(search-service)이라,
    ...
    reports {
        filters {
            excludes {
                ...
                classes("demo.search.SearchApplication*")
            }
        }
