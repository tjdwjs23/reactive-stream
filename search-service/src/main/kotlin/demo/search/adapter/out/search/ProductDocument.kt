...
// @Setting: Nori + ICU 초성 분석기 정의를 product-settings.json에서, @Mapping: name(+ name.chosung/name.jamo/name.ngram 다중필드)을
// product-mappings.json에서 로드합니다. 실제 필드 매핑은 그 JSON이 최종 권위이며, name의 초성/부분 서브필드도 거기서 정의됩니다.
// 문서는 name만 저장하고, name.chosung(초성 + edge_ngram)/name.jamo(전체 자모 + edge_ngram)/name.ngram(접두 edge_ngram)은 색인 시 분석기가 파생합니다(multi-field).
